...
router.post('/upload', auth_middleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { firId, filename, mimetype, size, documentType, content, // base64 encoded file content
         } = req.body;
        ...
        const encryptedContent = await (0, security_1.encryptData)(content);
        const document = await prisma_1.prisma.document.create({
            data: {
                firId: String(firId),
                uploadedById: userId,
                filename: (0, security_1.sanitizeInput)(filename),
                mimetype,
...
router.get('/:id', auth_middleware_1.authenticateToken, async (req, res) => {
    try {
        ...
        const decryptedContent = await (0, security_1.decryptData)(document.encryptedContent);
        await (0, audit_logger_1.logAudit)({
...
router.get('/fir/:firId', auth_middleware_1.authenticateToken, async (req, res) => {
    try {
        const userId = req.user.userId;
        const userRole = req.user.role;
...
router.post('/:id/verify', auth_middleware_1.authenticateToken, (0, auth_middleware_1.requireRole)(['OFFICER', 'SHO', 'ADMIN', 'SUPER_ADMIN']), async (req, res) => {
    try {
        const userId = req.user.userId;
        const userRole = req.user.role;
        const ipAddress = getIp(req);
