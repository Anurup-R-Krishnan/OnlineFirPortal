...
exports.sendFIRConfirmationEmail = sendFIRConfirmationEmail;
exports.sendFIRStatusUpdateEmail = sendFIRStatusUpdateEmail;
const resend_1 = require("resend");
// Initialize Resend with API key from environment
const resend = new resend_1.Resend(process.env.RESEND_API_KEY || 're_demo_key');
 ...
 */
async function sendFIRConfirmationEmail(to, userName, firNumber) {
    try {
        if (process.env.NODE_ENV === 'development' || !process.env.RESEND_API_KEY) {
 ...
 */
async function sendFIRStatusUpdateEmail(to, userName, firNumber, oldStatus, newStatus) {
    try {
        if (process.env.NODE_ENV === 'development' || !process.env.RESEND_API_KEY) {
 ...
 */
function generateFIRConfirmationHTML(userName, firNumber) {
    return `
<!DOCTYPE html>
<html>
<head>
 ...
 */
function generateFIRConfirmationText(userName, firNumber) {
    return `
Dear ${userName},
 ...
 */
function generateStatusUpdateHTML(userName, firNumber, oldStatus, newStatus) {
    return `
<!DOCTYPE html>
<html>
<head>
 ...
 */
function generateStatusUpdateText(userName, firNumber, oldStatus, newStatus) {
    return `
Dear ${userName},
